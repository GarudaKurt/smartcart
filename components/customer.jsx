...
import { collection, query, where, getDocs, updateDoc, doc,addDoc } from 'firebase/firestore';
  ...
  const EventCard = ({ event, onUpdateQuantity }) => {
    ...
    return (
      //I want here to get all the card data to be store to my createData() when ever the place order button is press
 ...
 const Customer = () => {
    ...
    const setQR = () =>{
      if(isOrdered){
        return <QRCode value={createBarcodeId()} size={200}/> //why was not able to display the QRCode svg ?
      }
      return null
    }
